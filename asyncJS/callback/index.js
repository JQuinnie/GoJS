...
document.getElementById('city').value = 'Houston, TX';
load();
...
function load() {
  ...
  var appid = '529508ab876efdef70c2a7a4950ebf43';
  ...
  function showResultsIfReady() {
    if (weatherData && fiveDayData) {
      console.log('success retrieving both results');
      showResults(buildCurrentSummary(weatherData));
      appendResults(buildForecastTable(fiveDayData));
    } else {
      console.log('not successful retrieving both results');
    }
  }
...
function buildForecastTable(fiveDay) {
  ...
  for (let record of fiveDay.list) {
    ...
    let newDay =
      !previous ||
      record.dt_txt.substring(0, 10) !== previous.dt_txt.substring(0, 10);
    if (newDay) {
      result += `<tr><td colspan=4><h3>${record.dt_txt.substring(
        0,
        10
      )}</h3></td></tr>`;
    }
