...
suite('Functional Tests', () => {
  ...
  // ### EXAMPLE ###
  test('Asynchronous test #example', (done) => { /** <= Pass a callback to the test function * */
    setTimeout(() => {
  ...
  suite('Integration tests with chai-http', () => {
    ...
    suite('GET /hello?name=[name] => "hello [name]"', () => {
      ...
      test('#example - ?name=John', (done) => { // Don't forget the callback...
        chai.request(server) // 'server' is the Express App
          .get('/hello?name=John') // http_method(url)
          .end((err, res) => { // Send the request. Pass a callback in
      ...
      test('Test GET /hello with no name', (done) => { // Don't forget the callback...
        chai.request(server) // 'server' is the Express App
          .get('/hello') // http_method(url). NO NAME in the query !
          .end((err, res) => { // res is the response object
      ...
      /**  Another one... * */
      test('Test GET /hello with your name', (done) => { // Don't forget the callback...
        chai.request(server) // 'server' is the Express App
          .get('/hello?name=JC') /** <=== Put your name in the query * */
          .end((err, res) => { // res is the response object
            // Your tests here.
            // Replace assert.fail(). Make the test pass.
            // Test the status and the text response. Follow the test order like above.
            assert.equal(res.status, 200);
            assert.equal(res.text, 'hello JC'/** <==  Put your name here * */);
            done(); // Always call the 'done()' callback when finished.
          });
      });
    });
    ...
    suite('PUT /travellers', () => {
      test('#example - responds with appropriate JSON data when sending {surname: "Polo"}', (done) => {
        chai.request(server)
          ...
          .end((err, res) => { // Send the request. Pass a Node callback
            assert.equal(res.status, 200, 'response status should be 200');
    ...
    Browser.site = 'https://foil-haddock.glitch.me/';
    ...
    suite('e2e Testing with Zombie.js', () => {
      ...
      suiteSetup(done => browser.visit('/', done), // Browser asynchronous operations take a callback
      );
